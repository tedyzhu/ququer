...
- [ ] 1. 搭建 `title_controller_test.js` 测试骨架与 fakePage 上下文替身
...
- [ ] 2. 固化 `updateDynamicTitle` 决策矩阵(去重阈值 >3)
...
- [ ] 3. 固化 `updateDynamicTitleWithRealNames` 决策矩阵(去重阈值 >2 + receiverTitleLocked 转交)
...
- [ ] 4. 固化 `updateTitleForReceiver` 决策矩阵(仅 B 端 + URL 解码 + 兜底重试)
...
- [ ] 5. 固化 `updateReceiverTitleWithRealNames` / `fetchRealInviterNameAndUpdateTitle` / `protectReceiverTitle`
...
- [ ] 7. 收敛 title-controller.js 的 S1/S2/S3(三处数组)
...
- [ ] 7.1 收敛 title-controller.js 的 S4(组合标题子串判断 → 提取昵称再判定)
...
- [ ] 8. 收敛 join-by-invite.js(S5 + L140 区)与 participant-listener.js(S6)
...
- [ ] 9. 收敛 title-controller.js 未点名的 4 组 inline 判定
